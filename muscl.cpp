...
std::vector<double> muscl_flux(state CLL, state CL, state CR, state CRR,
                   ...
                   double dx, double dt, double w, int l)
{
    ...
    {
        std::vector<double> FL = muscl_flux(X0[i], X0[i+1], X0[i+2], X0[i+3],
                                            dx, dt, w, l);
        std::vector<double> FR = muscl_flux(X0[i+1], X0[i+2], X0[i+3], X0[i+4],
                                            dx, dt, w, l);
                         ...
                         double dx, double C, double t, double w, int l)
{
    ...
    while (t0 < t)
    {
        ...
        muscl_stepper(ret, X0, dx, dt, w, l);
        t0 += dt;
        iter++;
    }
    return ret;
